...
select _ from t where no = 1 for update nowait → 대기없이 Exception을 던짐 select _ from t where no = 1 for update wait 3 → 3초 대기 후 Exception을 던짐
...
Sch-M(Schema Modification) : 테이블 구조를 변경하는 DDL 문을 수행할 때 발생하며, 수정 중인 스키마 정보를 다른 세션이 참조하지 못하도록 함 6) Bulk Update Lock
...
Sch-M은 어떤 Lock과도 호환되지 않는다. 3. Oracle Lock
