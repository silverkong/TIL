...
부분범위처리 원리 활용
효과적인 화면 페이지 처리
사용자 정의 함수/프로시저/트리거의 적절한 활용 2) Recursive Call
...
[그림 Ⅲ-1-11] ArraySize, Fetch Count, 블록 I/O관계
...
모든 결과집합을 다 읽을 때까지 2~3번 과정을 반복한다. 5. 페이지 처리 활용
...
void pagination(ResultSet rs, long pageNo, int pageSize) throws Exception { int i = 0 ; while( rs.next() ) { if(++i > (pageNo-1)_pageSize) printRow(rs); if(i == pageNo _ pageSize) break; } }
...
select /_+ driving_site(b) _/ channel_id, sum(quantity_sold) auantity_cold from order a, sales@lk_sales b where a.order_date between :1 and :2 and b.order_no = a.order no group by channel_id Rows Row Source Operation ---- --------------------------------------------- 5 SORT GROUP BY 10981 NESTED LOOPS 939 TABLE ACCESS (BY INDEX ROWID) OF ‘ORDER’ 939 INDEX (RANGE SCAN) OF ‘ORDER_IDX2’ (NON-UNIQUE) 10981 REMOTE
...
create table 일자 as select trunc(sysdate-rownum+1) d_date, to_char(trunc(sysdate-rownum+1), 'yyyymmdd') c_date from big_table where rownum <= (trunc(sysdate)-trunc(add_months(sysdate, - (12\*50)), 'yy')+1); create unique index 일자검사\_idx on 일자검사(c_date);
