...
비용기반(Cost-based) 쿼리 변환 : 변환된 쿼리의 비용이 더 낮을 때만 그것을 사용하고, 그렇지 않을 때는 원본 쿼리 그대로 두고 최적화를 수행한다. 2. 서브쿼리 Unnesting
...
분석 함수(Analytic Function) 4. 조건절 Pushing
